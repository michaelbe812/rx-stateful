...
export class NonFlickerComponent {
  ...
  value$ = this.query$.pipe(switchMap(() => this.client.request(this.fetch()).pipe(map((v) => v.value))));
  ...
  readonly page$ = this.page$$.pipe(scan((acc, curr) => acc + curr, 0));
  ...
  fetchPage(params: { delayInMs: number; page: number }) {
